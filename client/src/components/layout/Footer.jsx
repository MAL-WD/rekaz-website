...
const Footer = () => {
  ...
  return (
    <footer className="bg-rekaz-black text-white pt-20 pb-8 font-satoshi relative overflow-hidden">
      {/* Decorative top border gradient line */}
      <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-[#00a5ff]/40 to-transparent" />
      
      <div className="max-w-container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 mb-14">
          {/* Brand & Newsletter */}
          <div className="lg:col-span-2 text-start flex flex-col items-start">
            ...
            <div className="mb-6 w-full max-w-sm">
              <h4 className="font-semibold text-[15px] mb-3 text-white">{t('footer.newsletter', 'Subscribe to our newsletter')}</h4>
              <form className="flex gap-2" onSubmit={(e) => e.preventDefault()}>
                <input
                  type="email"
                  placeholder={t('footer.emailPlaceholder', 'Enter your email')}
                  className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-[13px] w-full text-white placeholder:text-gray-500 focus:outline-none focus:border-rekaz-cyan focus:ring-1 focus:ring-rekaz-cyan transition-all"
                />
                <button
                  type="submit"
                  className="bg-rekaz-gradient text-white px-5 py-2.5 rounded-xl text-[13px] font-semibold hover:opacity-90 transition-opacity"
                >
            ...
            <div className="flex gap-2.5">
              <a href="#" className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-rekaz-blue hover:scale-105 transition-all text-white border border-white/5">
          ...
          <div className="text-start">
            <h4 className="font-semibold text-[15px] tracking-wider uppercase text-white/55 mb-5">{t('footer.programsTitle', 'Programs')}</h4>
            <ul className="space-y-3 text-[14px] text-gray-400">
